...
def test_config_attrs(make_builder):
    ...
    cases = [
        ...
        ('datadir', '${srcdir}/data', os.path.join(SRCDIR, 'data')),
        ('helpersdir', '${srcdir}/helpers', os.path.join(SRCDIR, 'helpers')),
        ('cachedir', eib.CACHEDIR, eib.CACHEDIR),
        ('tmpdir', '${cachedir}/tmp', os.path.join(eib.CACHEDIR, 'tmp')),
        ('contentdir', '${cachedir}/content',
         os.path.join(eib.CACHEDIR, 'content')),
        ('outrootdir', '${tmpdir}/out', os.path.join(eib.CACHEDIR, 'tmp/out')),
        ('outdir', '${outrootdir}/${personality}',
         os.path.join(eib.CACHEDIR, 'tmp/out/base')),
        ('sysconfdir', eib.SYSCONFDIR, eib.SYSCONFDIR),
        ('build_version', '000101-000000', '000101-000000'),
        ('outversion',
         ('${product}-${branch}-${arch}-${platform}.${build_version}'
          '.${personality}'),
         'eos-master-amd64-amd64.000101-000000.base'),
        ('tmpconfig', '${tmpdir}/config.ini',
         os.path.join(eib.CACHEDIR, 'tmp/config.ini')),
        ('tmpfullconfig', '${tmpdir}/fullconfig.ini',
         os.path.join(eib.CACHEDIR, 'tmp/fullconfig.ini')),
        ('baselib', '${srcdir}/lib/eib.sh',
         os.path.join(SRCDIR, 'lib/eib.sh')),
        ('ssh_options',
         '-i ${sysconfdir}/ssh-key.pem -o StrictHostKeyChecking=no',
         ('-i {}/ssh-key.pem -o StrictHostKeyChecking=no'
          .format(eib.SYSCONFDIR))),
        ('keysdir', '${datadir}/keys', os.path.join(SRCDIR, 'data/keys')),
        ('keyring', '${tmpdir}/eib-keyring.gpg',
         os.path.join(eib.CACHEDIR, 'tmp/eib-keyring.gpg')),
        ('manifestdir', '${tmpdir}/manifest',
         os.path.join(eib.CACHEDIR, 'tmp/manifest')),
    ]
...
def test_set_environment(make_builder, monkeypatch):
    ...
    with monkeypatch.context() as m:
        ...
        cases = [
            ...
            ('EIB_SRCDIR', SRCDIR),
            ('EIB_DATADIR', os.path.join(SRCDIR, 'data')),
            ('EIB_HELPERSDIR', os.path.join(SRCDIR, 'helpers')),
            ('EIB_CACHEDIR', eib.CACHEDIR),
            ('EIB_TMPDIR', os.path.join(eib.CACHEDIR, 'tmp')),
            ('EIB_CONTENTDIR', os.path.join(eib.CACHEDIR, 'content')),
            ('EIB_OUTROOTDIR', os.path.join(eib.CACHEDIR, 'tmp/out')),
            ('EIB_OUTDIR', os.path.join(eib.CACHEDIR, 'tmp/out/base')),
            ('EIB_SYSCONFDIR', eib.SYSCONFDIR),
            ('EIB_BUILD_VERSION', '000101-000000'),
            ('EIB_OUTVERSION', 'eos-master-amd64-amd64.000101-000000.base'),
            ('EIB_TMPCONFIG', os.path.join(eib.CACHEDIR, 'tmp/config.ini')),
            ('EIB_TMPFULLCONFIG',
             os.path.join(eib.CACHEDIR, 'tmp/fullconfig.ini')),
            ('EIB_BASELIB', os.path.join(SRCDIR, 'lib/eib.sh')),
            ('EIB_SECT_OPT', 'a\n\tb'),
        ]
